 ...
 */
	...
	bl	board_init_f
		/*
		->spl_boot.c中board_init_f 		//common/board_f.c中也定义了board_init_f函数，怎么调用的这里不太明白
			//这里假设调用的spl_boot.c中board_init_f函数
...
--------------------------------------------------------------------------------------------------*/
	//接下来这两个函数就和uboot的设备树有关，新版uboot加入了设备树
	//和linux一样，u-boot这里也建立了一个驱动模型。比如， of_match来匹配，probe来识别等。
	#ifdef CONFIG_DM
		initr_dm,->
			dm_init_and_scan(false);
				dm_init()
					device_bind_by_name();					//根据名字绑定
						device_bind_common()						//核心，其与uclass建立关系里面调用uclass_bind_device
					device_probe()
				dm_scan_platdata();							//解析设备树信息并保存
					lists_bind_drivers();
						device_bind_by_name()
				dm_extended_scan_fdt();
					dm_scan_fdt()										
					dm_scan_fdt_node()							//分解子节点
				dm_scan_other();
			dm_timer_init()
	#endif
	#ifdef CONFIG_MMC
				...
				mmc_initialize(gd->bd);->
					mmc_probe(bis);->
						board_mmc_init(bis)
							...
							init_dwmmc();
								exynos_dwmmc_init()
									fdtdec_find_aliases_for_id()				//查找EXYNOS_DWMMC类型
